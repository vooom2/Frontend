...
export default function DocumentUpload({ handleStepChange }: { handleStepChange: (step: number) => void }) {
    const [driverLicense, setDriverLicense] = useState<File | null>(null);
    const [otherDocument, setOtherDocument] = useState<File | null>(null);
    const [previews, setPreviews] = useState<{ facial: string, other: string }>({ facial: '', other: '' });
    const [isLoading, setIsLoading] = useState(false);
    const facialInputRef = useRef<HTMLInputElement>(null);
    const otherInputRef = useRef<HTMLInputElement>(null);
    const updatePrimaryID = useVerificationStore((state) => state.updatePrimaryID);
    const updateSecondaryID = useVerificationStore((state) => state.updateSecondaryID);

    const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>, isFacial: boolean) => {
    ...
    const handleImagesUpload = async () => {
        if (driverLicense == null || otherDocument == null) { notify("Please select missing photo", "error"); return; };
        try {
            setIsLoading(true);
    ...
    return (
        <div className="flex flex-col items-center justify-center bg-gray-50 md:px-4 my-10">
            <div className="w-full max-w-3xl space-y-8">
                ...
                <div className="space-y-4">
                    <h3 className="text-lg font-medium text-center">Submit one of these documents</h3>
                    <div className="grid grid-cols-1 gap-4">
                        {renderUploadArea(false, "NIN card/slip | Voter's card | International passport")}
                    </div>
                </div>
            </div>
            <button
                className="max-w-3xl w-full mx-auto bg-black text-white py-3 rounded-xl hover:bg-gray-800 transition mt-6 flex justify-center"
