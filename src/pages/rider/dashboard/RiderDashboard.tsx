...
export default function RiderDashboard() {
  ...
  const makePayment = async (id: string) => {
    try {
      setIsLoading(true);
      const res = await UserService.makePayment(id) as { data: { authorization_url: string } };
      if (res) {
          ...
          {!pendingVehicle && (
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
              <DashboardInfoCard
                ...
                value={
                  stats
                    ? formatCurrency(parseInt(stats?.total_payments))
                    : null
                }
              />
              <DashboardInfoCard
                ...
                icon="clock"
                value={
                  stats ? formatCurrency(parseInt(stats?.weekly_due)) : null
                }
              />
              <DashboardInfoCard
                label="Outstanding"
                ...
                value={
                  stats
                    ? formatCurrency(parseInt(stats?.outstanding_payments))
                    : null
                }
              />
              <DashboardInfoCard
                ...
                icon="clock"
                value={stats ? stats?.inspection_count.toString() : null}
              />
            </div>
          )}
                          ...
                          <TableRow key={index}>
                            ...
                            <TableCell>
                              <Badge
                                ...
                                className={`capitalize  ${payment.payment_status !== "paid"
          ...
          {userInfo.account_verified && !vehicle && vehicleDetailsLoaded && (
            <div className="text-center px-4 my-10">
              ...
              <h2 className="text-xl md:text-2xl font-bold">
                Just hold a little longer, you will be assigned a bike!
              </h2>
            </div>
          )}

          {userInfo.account_verified && pendingVehicle && (
