...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      const res = (await OwnerServices.getOwnerVehicles()) as {
        vehicles: any;
      };
      console.log(res);
      if (res != null) {
    ...
    const fetchWalletInfo = async () => {
      const res = (await WalletServices.getOwnerWalletStat()) as {
        data: any;
      };
                            ...
                            <TableRow>
                              ...
                              >
                                <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-center">
                                  <div className="rounded-full bg-onprimary/10 md:p-6 p-4 mb-4 w-fit">
                                    <PackageXIcon className=" h-12 w-12 text-onprimary/60" />
                                  </div>
                                  <h3 className="text-base text-gray-400  mb-3">
                                    You have no vehicles listed on {APP_NAME}
                                  </h3>
                                  <Link to="host">
                                    <Button className="rounded-3xl bg-black px-4 py-2 text-sm ">
                                      Host your vehicle
                                    </Button>
                                  </Link>
                                </div>
